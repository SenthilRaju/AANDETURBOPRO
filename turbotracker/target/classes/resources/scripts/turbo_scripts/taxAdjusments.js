...
$( "#ogtaxterritoryoriginal" ).autocomplete({ minLength: 2,timeout :1000,
	select: function( event, ui ) { var id = ui.item.id; $("#ogtaxterritoryid").val(id); $("#ogTaxValue").html(ui.item.taxValue+"%"); taxAmountCalc("og",ui.item.taxValue);},
	source: function( request, response ) { var term = request.term;
		if ( term in cache ) { response( cache[ term ] ); 	return; 	}
		...
		$.ajax({
			...
			success: function(data) {
					...
					if(data.cuInvoiceId!=null)
					{
						...
						$("#taxfreight").val(data.taxfreight);
...
function taxAmountCalc(opt,tax)
{
	...
	console.log(taxRate);
		...
		{
		jQuery("#ogtax").val("");
		jQuery("#ogtax").val(formatCurrency((Number(ogsubamounttext)+Number(ogfrieghttext))*(Number(tax)/100)));	
		grandTotalCalculation(opt);
		}
}
...
{
	var cramounttext = jQuery("#cramounttext").val().replace(/[^0-9\.]+/g,"");
	var crfrieghttext = jQuery("#crfrieghttext").val().replace(/[^0-9\.]+/g,"");
	var crtaxAmount = jQuery("#crtax").val().replace(/[^0-9\.]+/g,"");
	
	console.log(crtaxAmount);
