...
function loadAdvancedSearchList(aAdvancedSearchSeri){
	...
	//	postData: aAdvancedSearchSeri,
		...
		colNames:['joMasterId','BidDate','Job #','Customer PO', 'Project','City', 'Salesman', 'Take Off', 'Quote By',  'Rep', 'Status', 'Customer', 'Division','Rep. #', 'Bid List'],
		colModel :[
           	...
           	{name:'joMasterId', index:'joMasterId', align:'left',sorttype:'date', width:50, editable:true,hidden:true, edittype:'text',
	           					editrules:{edithidden:false,required:false}},
           	{name:'bidDate', index:'bidDate', align:'left',sorttype:'date', width:50, editable:true,hidden:false, edittype:'text',
	           					editrules:{edithidden:false,required:false}},
	        {name:'jobNumber', index:'jobNumber', align:'left', width:50, sorttype:'text', editable:true,hidden:false, edittype:'text',
	           					editrules:{edithidden:false,required:false}},
			{name:'customerPONumber', index:'customerPONumber', align:'center',sorttype:'text', width:80, hidden:false, editable:true, edittype:'textarea', 
	    						editoptions:{}, editrules:{edithidden:true,required:false}},
			{name:'description', index:'description', align:'left', sorttype:'text', width:140,hidden:false, editable:true, cellattr: function (rowId, tv, rawObject, cm, rdata)	 {return 'style="white-space: normal" ';},
							editoptions:{size:20,readonly:false, alignText:'right'},editrules:{edithidden:true,required:true}},
			{name:'locationCity', index:'locationCity', align:'',sorttype:'text', width:80,hidden:false, editable:true,
								editoptions:{size:20,readonly:false, alignText:'right'},editrules:{edithidden:true,required:true}},
			{name:'initials', index:'ts.Initials', align:'center',sorttype:'text', width:40,hidden:false, editable:true,
							editoptions:{size:20,readonly:false, alignText:'right'},editrules:{edithidden:true,required:true}},
			{name:'takeoff', index:'takeoff', align:'center', sorttype:'text',width:40,hidden:false, editable:true,
							editoptions:{size:20,readonly:false, alignText:'right'},editrules:{edithidden:true,required:true}},
			{name:'quoteBy', index:'quoteBy', align:'center',sorttype:'text', width:40,hidden:false, editable:true,
							editoptions:{size:20,readonly:false, alignText:'right'},editrules:{edithidden:true,required:true}},
			{name:'initials', index:'initials', align:'center',sorttype:'text', width:80,hidden:true, editable:true,
							editoptions:{size:20,readonly:false, alignText:'right'},editrules:{edithidden:true,required:true}},
			{name:'jobStatus', index:'jobStatus', align:'center', sorttype:'text',width:65,hidden:false, editable:true,
							editoptions:{size:20,readonly:false, alignText:'right'},editrules:{edithidden:true,required:true}},
			{name:'customerName', index:'customerName', align:'',sorttype:'text', width:140,hidden:true, editable:true, cellattr: function (rowId, tv, rawObject, cm, rdata)	 {return 'style="white-space: normal" ';},
							editoptions:{size:20,readonly:false, alignText:'right'},editrules:{edithidden:true,required:true}},
			{name:'division', index:'division', align:'center',sorttype:'text', width:80, hidden:true, editable:true, edittype:'textarea', 
							editoptions:{}, editrules:{edithidden:true,required:false}},
			{name:'bidListCount', index:'bidListCount', align:'center',sorttype:'text', width:180,hidden:true, editable:true,
							editoptions:{size:20,readonly:false, alignText:'right'},editrules:{edithidden:true,required:true}},
			{name:'bidList', index:'bidList', align:'center', sorttype:'text',width:120,hidden:false, editable:true, cellattr: function (rowId, tv, rawObject, cm, rdata)	 {return 'style="white-space: normal" ';}, formatter: showBidderName,
							editoptions:{size:20,readonly:false, alignText:'right'},editrules:{edithidden:true,required:true}}
		],
		rowNum: 5000,
		pgbuttons: false,	
		...
		sortname: '', sortorder: 'asc',	imgpath: 'themes/basic/images',	caption: 'Resulted Jobs',
		// autowidth: true,
			...
			if(jobStatus != "Booked" && jobStatus != "Closed" && jobStatus != "Submitted"){
				$.ajax({
					...
					success: function(data){ 
						...
						var aQryStr = "jobNumber=" + jobNumber + "&jobName=" + urijobname + "&jobStatus=" + jobStatus+"&joMasterID="+joMasterID;
...
function showBidderName(cellValue, options, rowObject){
	...
	var jobNum = rowObject["jobNumber"];
	console.log('Status ' + status);
	if(aBidListCounr !== 1 && status !="Booked" && status !="Closed"){
		console.log('IF 1 : jobNum' + jobNum+' cellValue :' + cellValue); 
		cellValue = "Multiple";
	}else if(aBidListCounr === ''){
		console.log('Else IF  : jobNum' + jobNum+' cellValue :' + cellValue);
		return cellValue;
	}else{
		console.log('else : jobNum ' + jobNum +' cellValue :' + cellValue);
		return cellValue;
	}
	return cellValue;
}
