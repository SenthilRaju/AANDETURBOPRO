		...
		$
				.ajax({
					...
					data : bidderListValue + "&oper=" + aBidderDialogVar
							+ "&from=" + frompage,
					success : function(data) {
						$("#quotesBidlist").trigger("reloadGrid", [{page:1, current:true}]);
...
function openPDF(aQuotePDF) {
	...
	if (aQuotePDF === 'QuotePDF') {
		$.ajax({
			...
			url : "./quotePDFController/viewQuoteBidPdfForm",
			data : {
		...
		window.open("./quotePDFController/viewQuoteBidPdfForm?enginnerName="		
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (data.joQuoteHeaderId === null) {
					...
					setQuoteemailpopupdetails(aContactID);
