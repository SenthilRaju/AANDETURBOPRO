...
public class CustomerServiceImpl implements CustomerService {
	...
	public CuLinkageDetail getCuRecieptData(Integer cuMasterID,
		...
		if(!reverseStatus)
		{
		preLoadDataQuery = "SELECT DiscountUsed,creditusage, InvoiceApplied, InvoiceAmount FROM (SELECT sum(DiscountUsed) AS DiscountUsed,rxCustomerID FROM cuLinkageDetail  WHERE cuReceiptID = "+recieptID+" GROUP BY rxCustomerID)cd "
				+ "LEFT JOIN (SELECT sum(paymentApplied) AS creditusage,rxCustomerID FROM cuLinkageDetail WHERE PaymentApplied<0  AND cuReceiptID = "
				+ recieptID
				+ " GROUP BY rxCustomerID ) cdn ON cd.rxCustomerID = cdn.rxCustomerID "
				+ "LEFT JOIN (SELECT sum(paymentApplied) AS InvoiceApplied,rxCustomerID FROM cuLinkageDetail WHERE PaymentApplied>0  and cuReceiptID = "
				+ recieptID
