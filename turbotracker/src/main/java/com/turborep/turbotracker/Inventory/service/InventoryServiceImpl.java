...
public class InventoryServiceImpl implements InventoryService {
	...
	@Override
	public List<?> getInventory(int theFrom, int theTo, Integer warehouseId, String theSearchString, Integer inactive,
			String column, String sortBy) throws InventoryException {
		Session aSession = null;
		List<Prmaster> aQueryList = new ArrayList<Prmaster>();

		/*
		 * String aStr = "SELECT " + "prMaster.prMasterID, " +
		 * "prMaster.ItemCode, " + "prMaster.Description, " +
		 * "prMaster.InventoryOnHand, " + "prMaster.InventoryAllocated, " +
		 * "prMaster.InventoryOnOrder, " + "prMaster.Submitted  ";
		 */
		String itemcodeCondn = "";
		/*
		 * if(itemCode != null) itemcodeCondn=" OR pr.ItemCode LIKE '%"
		 * +itemCode+"%'";
		 */
		/*
		...
		itemcodeCondn = " AND ( pr.Description LIKE '%" + theSearchString + "%' OR" + " pr.ItemCode LIKE '%"
				+ theSearchString + "%' OR" + " pc.Description LIKE '%" + theSearchString + "%' OR"
				+ " pd.Description LIKE '%" + theSearchString + "%' OR" + " rx.Name LIKE '%" + theSearchString + "%') ";
		// }
		...
		if (theSearchString == null)
			theSearchString = "";
		// aStr = aStr+" FROM prMaster WHERE Description NOT LIKE '%DO NOT %'
		// AND ItemCode LIKE '%"+itemCode+"%' ORDER BY prMaster.ItemCode LIMIT "
		// + theFrom + ", " + theTo;
		// else
		// aStr = aStr+" FROM prMaster WHERE Description NOT LIKE '%DO NOT %'
		// ORDER BY prMaster.ItemCode LIMIT " + theFrom + ", " + theTo;
		...
		if (warehouseId == -1)
			aStr = "SELECT pr.prMasterID,pr.ItemCode,REPLACE (pr.Description,'``','\"') as prDescription, SUM(pw.InventoryOnHand) AS InventoryOnHand, "
					...
					+ "SUM(pw.InventoryOnOrder) AS InventoryOnOrder, " + "SUM(pr.Submitted) AS Submitted, "
					...
					+ " FROM prMaster pr" + " LEFT JOIN prWarehouseInventory pw ON pr.prMasterID = pw.prMasterID "
					...
					+ " LEFT JOIN rxMaster rx ON rx.rxMasterID = pr.rxMasterIDPrimaryVendor WHERE " + warehouseCondn
					+ " " + inactiveCondn + itemcodeCondn + " GROUP BY prMasterID ";
		else
		...
		try {
			...
			while (aIterator.hasNext()) {
				...
				// System.out.println(aPrmaster.getDescription()+" :::::
				// "+aPrmaster.getInventoryOnOrder()+ " :::
				// "+aPrmaster.getInventoryOnHand());
				if (aUnitCost != null) {
					Integer aUnit = Integer.valueOf(aUnitCost.intValue());
					if (aUnit != 0) {
						aTotal = aUnitCost == null
								? new BigDecimal("0.0000")
										.subtract(aAllocated == null ? new BigDecimal("0.0000") : aAllocated)
								: aUnitCost.subtract(aAllocated == null ? new BigDecimal("0.0000") : aAllocated);
	...
	public List<PrDepartment> getPrDepartment() throws InventoryException {
		...
		try {
			...
			itsLogger.error(excep.getMessage(), excep);
			InventoryException aInventoryException = new InventoryException(
					"Exception occurred while getting the Fried Charges in Purchase: " + excep.getMessage(), excep);
			throw aInventoryException;
		} finally {
			aSession.flush();
			aSession.close();
	...
	public ArrayList<AutoCompleteBean> getVendorList(String theSearchString) throws InventoryException {
		...
		String aSalesselectQry = "SELECT rxMasterID, Name FROM rxMaster where IsVendor = 1 AND Name IS NOT null  AND Name like '%"
				+ theSearchString + "%' ORDER BY Name ASC";
		Session aSession = null;
	...
	@Override
	public Boolean updateInventoryDetails(Prmaster thePrMaster, Prwarehouseinventory thePrwarehouseinventory)
		...
		try {
			...
			aTransaction = aSession.beginTransaction();
			aTransaction.begin();
			aPrwarehouseinventory = (Prwarehouseinventory) aSession.get(Prwarehouseinventory.class,
					thePrwarehouseinventory.getPrWarehouseInventoryId());
	...
	public Integer getPrWareID(Prwarehouseinventory thePrwarehouseinventory) throws InventoryException {
		...
		/*
		 * if(thePrwarehouseinventory.getPrWarehouseId() != null){ aWarehouseStr
		 * =
		 * "SELECT prWarehouseInventoryID FROM prWarehouseInventory WHERE prMasterID = "
		 * +thePrwarehouseinventory.getPrMasterId()+" and prWarehouseID = "
		 * +thePrwarehouseinventory.getPrWarehouseId()+""; }else{ aWarehouseStr
		 * =
		 * "SELECT prWarehouseInventoryID FROM prWarehouseInventory WHERE prMasterID = "
		 * +thePrwarehouseinventory.getPrMasterId()+""; }
		 */
		aWarehouseStr = "SELECT prWarehouseInventoryID FROM prWarehouseInventory WHERE prMasterID = "
				+ thePrwarehouseinventory.getPrMasterId() + "";
		Session aSession = null;
	...
	@Override
	public Prwarehouseinventory getPrwarehouseinventoryDetails(Integer PrwarehouseinventoryID)
			throws InventoryException {
		Session aSession = null;
		Prwarehouseinventory aPrwarehouseinventory = null;
		Integer aPrWareID = null;
	...
	@Override
	public Integer addNewInventory(Prmaster thePrMaster, Prwarehouseinventory thePrwarehouseinventory)
			throws InventoryException {
	...
	/*
	 * @Override public Integer updateAverageCost(Prmaster thePrMaster) throws
	 * InventoryException {
	 * 
	 * String aWarehouseStr = ""; aWarehouseStr =
	 * "SELECT TranDate, IsPurchase, QTY, Cost, FreightCost FROM ( SELECT P.OrderDate AS TranDate, 1 AS IsPurchase, "
	 * +
	 ...
	 * + "WHERE D.prMasterID ="+thePrMaster.getPrMasterId()+
	 * " AND P.prWarehouseID IS NOT NULL AND P.TransactionStatus > 0 UNION ALL SELECT H.InvoiceDate AS TranDate, "
	 * +
	 * "0 AS IsPurchase, D.QuantityBilled*-1 AS QTY, NULL AS Cost, NULL AS FreightCost FROM cuInvoice H RIGHT "
	 * +
	 * "OUTER JOIN cuInvoiceDetail D ON H.cuInvoiceID = D.cuInvoiceID WHERE (D.prMasterID = "
	 * +thePrMaster.getPrMasterId()+") AND (H.TransactionStatus > 0) " +
	 ...
	 * +
	 * "WHERE (prTransfer.prToWarehouseID IS NULL) AND (prTransferDetail.prMasterID ="
	 * +thePrMaster.getPrMasterId()+")) qry ORDER BY TranDate, IsPurchase DESC";
	 * Session aSession = null; Integer isPurchase = 0; BigDecimal qty = new
	 * BigDecimal("0.0000"); BigDecimal cost = new BigDecimal("0.0000");
	 * BigDecimal freightCost = new BigDecimal("0.0000"); try { If
	 * Logical(prMaster("HasInitialCost")) Then FirstCost = False TotalQty =
	 * TotalQty + Dollar(prMaster("InitialOnHand")) averageCost =
	 * Dollar(prMaster("InitialCost")) End If
	 * if(thePrMaster.isHasInitialCost()==1){
	 * 
	 * }
	 * 
	 * aSession = itsSessionFactory.openSession(); Query aQuery =
	 * aSession.createSQLQuery(aWarehouseStr); Iterator<?> iterator =
	 * aQuery.list().iterator(); while(iterator.hasNext()) { Object[] aObj =
	 * (Object[])iterator.next(); isPurchase = (Integer)aObj[1]; qty =
	 * (BigDecimal)aObj[2]; cost = (BigDecimal)aObj[3]; freightCost =
	 * (BigDecimal)aObj[4]; } } catch (Exception excep) {
	 * itsLogger.error(excep.getMessage(), excep); InventoryException
	 * aInventoryException = new InventoryException(excep.getMessage(), excep);
	 * throw aInventoryException; } finally { aSession.flush();
	 * aSession.close(); } return aPrWareID; }
	 */
	...
	/*
	 * @Override public List<PrDepartment> getInventoryDepartments() throws
	 * InventoryException {
	 * 
	 * return null; }
	 */
	...
	public ArrayList<AutoCompleteBean> getAccountList(String theSearchString, AutoCompleteBean theUserBean)
			throws InventoryException {
		String aAccountsselectQry = "SELECT coAccountID,Number,Description FROM coAccount where (Description IS NOT NULL  AND Description LIKE '%"
				+ theSearchString + "%') OR  (Number IS NOT NULL  AND Number LIKE '%" + theSearchString + "%')";
		Session aSession = null;
		ArrayList<AutoCompleteBean> aQueryList = new ArrayList<AutoCompleteBean>();
	...
	public ArrayList<SalesOrderBean> getsalesOrderList(Integer theprMasterID) throws InventoryException {
		// String aCusoselectQry = "SELECT
		// so.cuSOID,sodetail.cuSODetailID,so.CreatedOn,so.DatePromised,rxmaster.Name,so.joReleaseId,SUM(sodetail.quantityordered)
		// AS quantityordered,warehouse.City,so.rxCustomerID FROM cuSODetail
		// sodetail JOIN cuSO so ON so.cuSOID = sodetail.cuSOID JOIN rxMaster
		// rxmaster ON rxmaster.rxMasterID = so.rxCustomerID JOIN prWarehouse
		// warehouse ON warehouse.prWarehouseID = so.prFromWarehouseID WHERE
		// sodetail.prMasterID ="+theprMasterID+" GROUP BY sodetail.cuSOID ORDER
		// BY sodetail.cuSOID DESC";
		/*
		 * String aCusoselectQry =
		 * "SELECT so.cuSOID,sodetail.cuSODetailID,so.CreatedOn,so.DatePromised,rxmaster.Name,so.joReleaseId,(SUM(sodetail.QuantityOrdered) -  SUM(sodetail.QuantityBilled)) AS quantityordered,warehouse.City,so.rxCustomerID FROM cuSO so, cuSODetail sodetail,rxMaster rxmaster,prWarehouse warehouse WHERE"
		 * +
		 ...
		 * + " sodetail.prMasterID= "+theprMasterID+
		 * " AND sodetail.QuantityBilled IS NOT NULL GROUP BY sodetail.cuSOID ORDER BY sodetail.cuSOID DESC"
		 * ;
		 */
		// String aCusoselectQry="SELECT
		// so.cuSOID,sodetail.cuSODetailID,so.CreatedOn,so.DatePromised,rxmaster.Name,so.joReleaseId,(SUM(sodetail.QuantityOrdered)
		// - SUM(sodetail.QuantityBilled)) AS
		// quantityordered,warehouse.City,so.rxCustomerID FROM cuSO so,
		// cuSODetail sodetail,rxMaster rxmaster,prWarehouse warehouse "+
		// "WHERE so.cuSOID = sodetail.cuSOID AND so.TransactionStatus = 1 AND
		// rxmaster.rxMasterID = so.rxCustomerID AND warehouse.prWarehouseID =
		// so.prFromWarehouseID AND "+
		// "sodetail.prMasterID="+theprMasterID+" AND sodetail.QuantityBilled IS
		// NOT NULL GROUP BY sodetail.cuSOID"+
		// " UNION "+
		// "SELECT '-1' AS cuSOID,'-1' AS cuSODetailID, H.TransferDate AS
		// OrderDate, NULL AS DatePromised, 'Warehouse Transfer Out' AS NAME,''
		// AS joReleaseId,D.QuantityTransfered AS
		// QuantityOrdered,prWarehouse.City,'0' AS rxCustomerID FROM prWarehouse
		// INNER JOIN prWHtransfer H ON "+
		// "prWarehouse.prWarehouseID = H.prFromWareHouseID RIGHT OUTER JOIN
		// prWHtransferDetail D ON H.prTransferID = D.prTransferID WHERE
		// (H.ReceivedDate IS NULL) AND (D.prMasterID = "+theprMasterID+")";
		...
		String aCusoselectQry = "SELECT so.cuSOID,sodetail.cuSODetailID,so.CreatedOn,so.DatePromised,rxmaster.Name,so.SONumber,(SUM(sodetail.QuantityOrdered) - SUM(sodetail.QuantityBilled)) AS quantityordered,warehouse.SearchName,so.rxCustomerID FROM cuSO so LEFT JOIN cuSODetail sodetail ON (so.cuSOID = sodetail.cuSOID) "
				...
				+ "WHERE  so.TransactionStatus = 1 AND " + "sodetail.prMasterID=" + theprMasterID
				+ " AND sodetail.QuantityBilled IS NOT NULL AND pr.IsInventory=1  GROUP BY sodetail.cuSOID"
				+ " UNION ALL "
				...
				+ "prWarehouse.prWarehouseID = H.prFromWareHouseID RIGHT OUTER JOIN prWHtransferDetail D ON H.prTransferID = D.prTransferID LEFT JOIN prMaster pr ON(D.prMasterID=pr.prMasterID)  WHERE (H.ReceivedDate IS NULL) AND  `prToWareHouseID` IS NOT NULL AND pr.IsInventory=1 AND (D.prMasterID = "
				+ theprMasterID + ")";
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				if (aSalesOrderBean.getQuantityOrdered() != null
						&& aSalesOrderBean.getQuantityOrdered().compareTo(BigDecimal.ZERO) == 0) {
	...
	public ArrayList<PurchaseOrdersBean> getOnOrderList(Integer theprMasterID) throws InventoryException {
		...
		/*
		 * String aCusoselectQry =
		 * "SELECT vepo.vePOID,vepo.PONumber,vepo.OrderDate,vedetail.EstimatedShipDate,rxmaster.Name,vepo.rxVendorID,vepo.prWarehouseID,SUM(vedetail.QuantityOrdered) AS QuantityOrdered,warehouse.City FROM vePO vepo,vePODetail vedetail,rxMaster rxmaster,prWarehouse warehouse WHERE"
		 * +
		 * " vepo.vePOID = vedetail.vePOID AND rxmaster.rxMasterID = vepo.rxVendorID AND warehouse.prWarehouseID = vepo.prWarehouseID AND"
		 * + " vedetail.prMasterID= "+theprMasterID+
		 * " AND vedetail.QuantityOrdered IS NOT NULL AND vepo.receiveddate IS NULL AND vepo.vePOID NOT IN (SELECT vebill.vePOID FROM veBill vebill) "
		 * + "GROUP BY vepo.vePOID  ORDER BY vedetail.vePOID DESC";
		 */
		/*
		 * String aCusoselectQry =
		 * "SELECT vepo.vePOID,vepo.PONumber,vepo.OrderDate,vedetail.EstimatedShipDate,rxmaster.Name,vepo.rxVendorID,"
		 * +
		 ...
		 * +
		 * "LEFT JOIN veBill vebill ON vepo.vePOID = vebill.vePOID  WHERE  vedetail.prMasterID= "
		 * +theprMasterID + " AND  vedetail.QuantityOrdered IS NOT NULL  AND "
		 * //+
		...
		String aCusoselectQry = "SELECT vepo.vePOID,CAST(vepo.PONumber  AS CHAR) AS ponumber,vepo.OrderDate,vedetail.EstimatedShipDate,rxmaster.Name,vepo.rxVendorID,"
				...
				// +"SUM(IFNULL(vedetail.QuantityOrdered,0)-IFNULL(vedetail.QuantityBilled
				// ,0) ) END AS QuantityOrdered,"
				+ "warehouse.City FROM vePO vepo " + "LEFT JOIN vePODetail vedetail ON vepo.vePOID = vedetail.vePOID "
				+ "LEFT JOIN rxMaster rxmaster ON rxmaster.rxMasterID = vepo.rxVendorID "
				+ "LEFT JOIN prWarehouse warehouse ON warehouse.prWarehouseID = vepo.prWarehouseID "
				+ "LEFT JOIN joRelease ON vepo.joReleaseID=joRelease.joReleaseID "
				+ "LEFT JOIN prMaster pr ON(vedetail.prMasterID=pr.prMasterID) " + " WHERE  vedetail.prMasterID= "
				+ theprMasterID + " AND  vedetail.QuantityOrdered IS NOT NULL AND pr.IsInventory=1 "
				+ " And (joRelease.releaseType IS NULL || joRelease.releaseType<>4) "
				// + "AND (vepo.receiveddate IS NULL OR
				// (vedetail.QuantityOrdered-vedetail.QuantityReceived) >0) "
				+ "GROUP BY vepo.vePOID HAVING QuantityOrdered>0 " + "UNION All "
	...
	public ArrayList<PurchaseOrdersBean> getOnHandList(Integer theprMasterID) throws InventoryException {
		...
		// sodetail.prMasterID ="+theprMasterID+" GROUP BY sodetail.cuSOID ORDER
		// BY sodetail.cuSOID DESC";
		String aCusoselectQry = "SELECT inventory.InventoryOnHand,inventory.InventoryAllocated,inventory.InventoryOnOrder,warehouse.SearchName FROM prWarehouseInventory inventory,prWarehouse warehouse,prMaster pr  WHERE  inventory.prWarehouseID = warehouse.prWarehouseID AND inventory.prMasterID=pr.prMasterID AND inventory.prMasterID ="
				+ theprMasterID + " AND pr.IsInventory=1";
		Session aSession = null;
		ArrayList<PurchaseOrdersBean> aQueryList = new ArrayList<PurchaseOrdersBean>();
		PurchaseOrdersBean aPurchaseOrdersBean = null;
		try {
			...
			while (iterator.hasNext()) {
				...
				BigDecimal inventoryonhand = aPurchaseOrdersBean.getInventoryOnHand() == null ? new BigDecimal("0.0000")
						: aPurchaseOrdersBean.getInventoryOnHand();
				BigDecimal inventoryAllocated = aPurchaseOrdersBean.getInventoryAllocated() == null
						? new BigDecimal("0.0000") : aPurchaseOrdersBean.getInventoryAllocated();
				/*
				 * if(aPurchaseOrdersBean.getInventoryAllocated().compareTo(new
				 * BigDecimal("0.0000"))==1){
				 * aPurchaseOrdersBean.setInventoryAvailable(aPurchaseOrdersBean
				 * .getInventoryOnHand()==null?new
				 * BigDecimal("0.0000").subtract(aPurchaseOrdersBean.
				 * getInventoryAllocated()==null?new
				 * BigDecimal("0.0000"):aPurchaseOrdersBean.
				 * getInventoryAllocated()):aPurchaseOrdersBean.
				 * getInventoryOnHand().subtract(aPurchaseOrdersBean.
				 * getInventoryAllocated()==null?new
				 * BigDecimal("0.0000"):aPurchaseOrdersBean.
				 * getInventoryAllocated())); }else{
				 * aPurchaseOrdersBean.setInventoryAvailable(aPurchaseOrdersBean
				 * .getInventoryOnHand()); }
				 */
				...
				System.out.println("OnHand" + aPurchaseOrdersBean.getInventoryOnHand() + "\n Allocated"
						+ aPurchaseOrdersBean.getInventoryAllocated() + "\n OnOrder"
						+ aPurchaseOrdersBean.getInventoryOnOrder() + "\n Available"
						+ aPurchaseOrdersBean.getInventoryAvailable());
				aQueryList.add(aPurchaseOrdersBean);
			}
	...
	public ArrayList<PurchaseOrdersBean> getOnHandListForInventoryAdjust(Integer theprMasterID, Integer theWarehouseID)
			throws InventoryException {
		// String aCusoselectQry = "SELECT
		// so.cuSOID,sodetail.cuSODetailID,so.CreatedOn,so.DatePromised,rxmaster.Name,so.joReleaseId,SUM(sodetail.quantityordered)
		// AS quantityordered,warehouse.City,so.rxCustomerID FROM cuSODetail
		...
		// sodetail.prMasterID ="+theprMasterID+" GROUP BY sodetail.cuSOID ORDER
		// BY sodetail.cuSOID DESC";
		String aCusoselectQry = "SELECT inventory.InventoryOnHand,inventory.InventoryAllocated,inventory.InventoryOnOrder,warehouse.SearchName FROM prWarehouseInventory inventory,prWarehouse warehouse  WHERE prMasterID =  "
				+ theprMasterID
				+ " AND inventory.prWarehouseID = warehouse.prWarehouseID AND inventory.prWarehouseID != "
				+ theWarehouseID + "  ";
		Session aSession = null;
		ArrayList<PurchaseOrdersBean> aQueryList = new ArrayList<PurchaseOrdersBean>();
		PurchaseOrdersBean aPurchaseOrdersBean = null;
		try {
			...
			while (iterator.hasNext()) {
				...
				aPurchaseOrdersBean.setInventoryAllocated((BigDecimal) aObj[1]);
				aPurchaseOrdersBean.setInventoryOnOrder((BigDecimal) aObj[2]);
				aPurchaseOrdersBean.setInventoryAvailable(aPurchaseOrdersBean.getInventoryOnHand() == null
						? new BigDecimal("0.0000").subtract(aPurchaseOrdersBean.getInventoryAllocated() == null
								? new BigDecimal("0.0000") : aPurchaseOrdersBean.getInventoryAllocated())
						: aPurchaseOrdersBean.getInventoryOnHand()
								.subtract(aPurchaseOrdersBean.getInventoryAllocated() == null ? new BigDecimal("0.0000")
										: aPurchaseOrdersBean.getInventoryAllocated()));
				aPurchaseOrdersBean.setCity((String) aObj[3]);
				aQueryList.add(aPurchaseOrdersBean);
			}
	...
	public ArrayList<Prwhtransferdetail> getWarehouseTransferList(Integer theprTransferId) throws InventoryException {
		...
		// sodetail.prMasterID ="+theprMasterID+" GROUP BY sodetail.cuSOID ORDER
		// BY sodetail.cuSOID DESC";
		String aCusoselectQry = "select prTransferDetailID,prTransferID,prMasterID,Description,QuantityTransfered,QuantityAvailable,InventoryCost,ItemCode from prWHtransferDetail where prTransferID="
				+ theprTransferId;
		Session aSession = null;
	...
	public List<Prmaster> getTransferLineItems(Integer prMasterID, Integer fromwarehouse) throws InventoryException {
		...
		// String aQry = "SELECT
		// prMasterId,itemCode,Description,isTaxable,POMult,SalesPrice00,InventoryOnHand
		// FROM prMaster WHERE prMasterId = "+prMasterID;
		String aQry = "SELECT pm.prMasterId,pm.itemCode,pm.Description,pm.isTaxable,pm.POMult,pm.SalesPrice00,pw.InventoryOnHand,pw.InventoryAllocated FROM prMaster pm JOIN prWarehouseInventory pw ON(pm.prMasterID=pw.prMasterID)  WHERE pm.prMasterId = "
				+ prMasterID + " AND pw.`prWarehouseID`=" + fromwarehouse;
		Session aSession = null;
	...
	@Override
	public PrwarehouseTransfer updateTransferDetails(PrwarehouseTransfer aPrwarehouseTransfer)
			throws InventoryException {
		Session aPrWarehouseSession = itsSessionFactory.openSession();
		...
		try {
			aTransaction = aPrWarehouseSession.beginTransaction();
			aTransaction.begin();
			objPrwarehouseTransfer = (PrwarehouseTransfer) aPrWarehouseSession.get(PrwarehouseTransfer.class,
					aPrwarehouseTransfer.getPrTransferId());
	...
	public boolean saveTransferLineItems(PrwarehouseTransfer aPrwarehouseTransfer,
			Prwhtransferdetail aPrwhtransferdetail, String oper) throws InventoryException {
		Session aPrWarehouseSession = itsSessionFactory.openSession();
		...
		try {
			...
			if ("add".equalsIgnoreCase(oper)) {
				// updatePrMasterForWTransfer(aPrwhtransferdetail, new
				// BigDecimal(0),
				// aPrwhtransferdetail.getQuantityTransfered(), "add",null);
				/*
				 * updatePrWarehouseInventoryForWTransfer(aPrwhtransferdetail,
				 * new BigDecimal(0),
				 * aPrwhtransferdetail.getQuantityTransfered(), "add",null);
				 */
				...
				if (id > 0) {
					PrwarehouseTransfer thePrwarehouseTransfer = (PrwarehouseTransfer) aPrWarehouseSession
							.get(PrwarehouseTransfer.class, aPrwhtransferdetail.getPrTransferId());
					...
					Prmaster aPrmaster = productService
							.getProducts(" WHERE prMasterID=" + aPrwhtransferdetail.getPrMasterId());
					...
					aTpInventoryLog.setProductOut(
							aPrwhtransferdetail.getQuantityTransfered().compareTo(new BigDecimal("0.0000")) == 1
									? aPrwhtransferdetail.getQuantityTransfered() : new BigDecimal("0.0000"));
					...
					/*
					 * TpInventoryLogMaster Created on 04-12-2015 Code Starts
					 * FromWarehouseID AllocatedAffected
					 */
					Session alogSession = itsSessionFactory.openSession();
					BigDecimal qo = aPrwhtransferdetail.getQuantityTransfered().compareTo(new BigDecimal("0.0000")) == 1
							? aPrwhtransferdetail.getQuantityTransfered() : new BigDecimal("0.0000");
					Prwarehouse theprwarehouse = (Prwarehouse) alogSession.get(Prwarehouse.class,
							thePrwarehouseTransfer.getPrFromWarehouseId());
					Prwarehouseinventory theprwarehsinventory = itsInventoryService.getPrwarehouseInventory(
							thePrwarehouseTransfer.getPrFromWarehouseId(), aPrmaster.getPrMasterId());
					TpInventoryLogMaster prmatpInventoryLogMstr = new TpInventoryLogMaster(aPrmaster.getPrMasterId(),
							aPrmaster.getItemCode(), thePrwarehouseTransfer.getPrFromWarehouseId(),
							theprwarehouse.getSearchName(), aPrmaster.getInventoryOnHand(),
							theprwarehsinventory.getInventoryOnHand(), qo, BigDecimal.ZERO, "IT",
							thePrwarehouseTransfer.getPrTransferId(), aPrwhtransferdetail.getPrTransferDetailId(),
							thePrwarehouseTransfer.getTransactionNumber().toString(), null,
							/* Product out */(qo.compareTo(BigDecimal.ZERO) > 0) ? qo : BigDecimal.ZERO,
							/* Product in */(qo.compareTo(BigDecimal.ZERO) < 0) ? qo.multiply(new BigDecimal(-1))
									: BigDecimal.ZERO,
					 ...
					 */
					theprwarehouse = (Prwarehouse) alogSession.get(Prwarehouse.class,
							thePrwarehouseTransfer.getPrToWarehouseId());
					theprwarehsinventory = itsInventoryService.getPrwarehouseInventory(
							thePrwarehouseTransfer.getPrToWarehouseId(), aPrmaster.getPrMasterId());
					prmatpInventoryLogMstr = new TpInventoryLogMaster(aPrmaster.getPrMasterId(),
							aPrmaster.getItemCode(), thePrwarehouseTransfer.getPrToWarehouseId(),
							theprwarehouse.getSearchName(), aPrmaster.getInventoryOnHand(),
							theprwarehsinventory.getInventoryOnHand(), BigDecimal.ZERO, qo, "IT",
							thePrwarehouseTransfer.getPrTransferId(), aPrwhtransferdetail.getPrTransferDetailId(),
							thePrwarehouseTransfer.getTransactionNumber().toString(), null,
							/* Product out */(qo.compareTo(BigDecimal.ZERO) < 0) ? qo.multiply(new BigDecimal(-1))
									: BigDecimal.ZERO,
							/* Product in */(qo.compareTo(BigDecimal.ZERO) > 0) ? qo : BigDecimal.ZERO, "IT Created",
							aPrwhtransferdetail.getUserId(), aPrwhtransferdetail.getUserName(), new java.util.Date());
					itsInventoryService.addTpInventoryLogMaster(prmatpInventoryLogMstr);
					/* Code Ends */
				}
			} else if ("del".equalsIgnoreCase(oper)) {
				objPrwhtransferdetail = (Prwhtransferdetail) aPrWarehouseSession.get(Prwhtransferdetail.class,
						aPrwhtransferdetail.getPrTransferDetailId());
				// updatePrMasterForWTransfer(objPrwhtransferdetail, new
				// BigDecimal(0), new BigDecimal(
				// 0), "del",null);
				/*
				 * updatePrWarehouseInventoryForWTransfer(aPrwhtransferdetail,
				 * new BigDecimal(0), new BigDecimal( 0), "del",null);
				 */
				...
				if (aPrwhtransferdetail.getPrTransferDetailId() > 0) {
					PrwarehouseTransfer thePrwarehouseTransfer = (PrwarehouseTransfer) aPrWarehouseSession
							.get(PrwarehouseTransfer.class, aPrwhtransferdetail.getPrTransferId());
					Prwhtransferdetail thePrwhtransferdetail = (Prwhtransferdetail) aPrWarehouseSession
							.get(Prwhtransferdetail.class, aPrwhtransferdetail.getPrTransferDetailId());
					TpInventoryLog aTpInventoryLog = new TpInventoryLog();
					aTpInventoryLog.setPrMasterID(thePrwhtransferdetail.getPrMasterId());
					Prmaster aPrmaster = productService
							.getProducts(" WHERE prMasterID=" + thePrwhtransferdetail.getPrMasterId());
					aTpInventoryLog.setProductCode(aPrmaster.getItemCode());
					aTpInventoryLog.setWareHouseID(thePrwarehouseTransfer.getPrFromWarehouseId());
					aTpInventoryLog.setTransType("IT");
					...
					aTpInventoryLog.setProductIn(
							thePrwhtransferdetail.getQuantityTransfered().compareTo(new BigDecimal("0.0000")) == 1
									? thePrwhtransferdetail.getQuantityTransfered() : new BigDecimal("0.0000"));
					aTpInventoryLog.setProductOut(new BigDecimal("0.0000"));
					aTpInventoryLog.setUserID(thePrwarehouseTransfer.getCreatedByID());
					aTpInventoryLog.setCreatedOn(thePrwarehouseTransfer.getCreatedOn());
					itsInventoryService.saveInventoryTransactions(aTpInventoryLog);
					...
					/*
					 * TpInventoryLogMaster Created on 04-12-2015 Code Starts
					 * FromWarehouseID AllocatedAffected Rollback
					 */
					Session alogSession = itsSessionFactory.openSession();
					BigDecimal qo = thePrwhtransferdetail.getQuantityTransfered()
							.compareTo(new BigDecimal("0.0000")) == 1 ? thePrwhtransferdetail.getQuantityTransfered()
									: new BigDecimal("0.0000");
					Prwarehouse theprwarehouse = (Prwarehouse) alogSession.get(Prwarehouse.class,
							thePrwarehouseTransfer.getPrFromWarehouseId());
					Prwarehouseinventory theprwarehsinventory = itsInventoryService.getPrwarehouseInventory(
					...
					TpInventoryLogMaster prmatpInventoryLogMstr = new TpInventoryLogMaster(aPrmaster.getPrMasterId(),
							aPrmaster.getItemCode(), thePrwarehouseTransfer.getPrFromWarehouseId(),
							theprwarehouse.getSearchName(), aPrmaster.getInventoryOnHand(),
							theprwarehsinventory.getInventoryOnHand(), qo.multiply(new BigDecimal(-1)), BigDecimal.ZERO,
							"IT", thePrwarehouseTransfer.getPrTransferId(), aPrwhtransferdetail.getPrTransferDetailId(),
							thePrwarehouseTransfer.getTransactionNumber().toString(), null,
							/* Product out */(qo.compareTo(BigDecimal.ZERO) < 0) ? qo.multiply(new BigDecimal(-1))
									: BigDecimal.ZERO,
							/* Product in */(qo.compareTo(BigDecimal.ZERO) > 0) ? qo : BigDecimal.ZERO, "IT Deleted",
							aPrwhtransferdetail.getUserId(), aPrwhtransferdetail.getUserName(), new java.util.Date());
					itsInventoryService.addTpInventoryLogMaster(prmatpInventoryLogMstr);
					/* Code Ends */
					...
					/*
					 * TpInventoryLogMaster Created on 04-12-2015 Code Starts
					 * ToWarehouseID OrderAffected Rollback
					 */
					theprwarehouse = (Prwarehouse) alogSession.get(Prwarehouse.class,
							thePrwarehouseTransfer.getPrToWarehouseId());
					theprwarehsinventory = itsInventoryService.getPrwarehouseInventory(
					...
					prmatpInventoryLogMstr = new TpInventoryLogMaster(aPrmaster.getPrMasterId(),
							aPrmaster.getItemCode(), thePrwarehouseTransfer.getPrToWarehouseId(),
							theprwarehouse.getSearchName(), aPrmaster.getInventoryOnHand(),
							theprwarehsinventory.getInventoryOnHand(), BigDecimal.ZERO, qo.multiply(new BigDecimal(-1)),
							"IT", thePrwarehouseTransfer.getPrTransferId(), aPrwhtransferdetail.getPrTransferDetailId(),
							thePrwarehouseTransfer.getTransactionNumber().toString(), null,
							/* Product out */(qo.compareTo(BigDecimal.ZERO) > 0) ? qo : BigDecimal.ZERO,
							/* Product in */(qo.compareTo(BigDecimal.ZERO) < 0) ? qo.multiply(new BigDecimal(-1))
				...
				// updatePrMasterForWTransfer(aPrwhtransferdetail,
				// oldQuantityOrdered,
				// newQuantityOrdered, "edit",objPrwhtransferdetail);
				/*
				 * updatePrWarehouseInventoryForWTransfer(aPrwhtransferdetail,
				 * oldQuantityOrdered, newQuantityOrdered,
				 * "edit",objPrwhtransferdetail);
				 */
				...
				if (objPrwhtransferdetail.getPrTransferDetailId() > 0
						&& oldQuantityTransfered.compareTo(newQuantityOrdered) != 0) {
					PrwarehouseTransfer thePrwarehouseTransfer = (PrwarehouseTransfer) aPrWarehouseSession
							.get(PrwarehouseTransfer.class, aPrwhtransferdetail.getPrTransferId());
					TpInventoryLog aTpInventoryLog = new TpInventoryLog();
					aTpInventoryLog.setPrMasterID(aPrwhtransferdetail.getPrMasterId());
					...
					aTpInventoryLog.setTransID(aPrwhtransferdetail.getPrTransferId());
					aTpInventoryLog.setTransDetailID(aPrwhtransferdetail.getPrTransferDetailId());
					aTpInventoryLog.setProductIn(oldQuantityTransfered.compareTo(new BigDecimal("0.0000")) == 1
							? oldQuantityTransfered : new BigDecimal("0.0000"));
					aTpInventoryLog.setProductOut(new BigDecimal("0.0000"));
					aTpInventoryLog.setUserID(thePrwarehouseTransfer.getCreatedByID());
					aTpInventoryLog.setCreatedOn(thePrwarehouseTransfer.getCreatedOn());
					itsInventoryService.saveInventoryTransactions(aTpInventoryLog);
					aTpInventoryLog.setWareHouseID(thePrwarehouseTransfer.getPrToWarehouseId());
					aTpInventoryLog.setProductOut(oldQuantityTransfered.compareTo(new BigDecimal("0.0000")) == 1
							? oldQuantityTransfered : new BigDecimal("0.0000"));
					aTpInventoryLog.setProductIn(new BigDecimal("0.0000"));
					itsInventoryService.saveInventoryTransactions(aTpInventoryLog);
					 ...
					 */
					Session alogSession = itsSessionFactory.openSession();
					BigDecimal qo = oldQuantityTransfered.compareTo(new BigDecimal("0.0000")) == 1
							? oldQuantityTransfered : new BigDecimal("0.0000");
					Prwarehouse theprwarehouse = (Prwarehouse) alogSession.get(Prwarehouse.class,
							thePrwarehouseTransfer.getPrFromWarehouseId());
					Prwarehouseinventory theprwarehsinventory = itsInventoryService.getPrwarehouseInventory(
					...
					TpInventoryLogMaster prmatpInventoryLogMstr = new TpInventoryLogMaster(aPrmaster.getPrMasterId(),
							...
							/* Product out */(qo.compareTo(BigDecimal.ZERO) < 0) ? qo.multiply(new BigDecimal(-1))
									: BigDecimal.ZERO,
							/* Product in */(qo.compareTo(BigDecimal.ZERO) > 0) ? qo : BigDecimal.ZERO, "IT Edited",
							aPrwhtransferdetail.getUserId(), aPrwhtransferdetail.getUserName(), new java.util.Date());
					itsInventoryService.addTpInventoryLogMaster(prmatpInventoryLogMstr);
					/* Code Ends */
					...
					/*
					 * TpInventoryLogMaster Created on 04-12-2015 Code Starts
					 * FromWarehouseID AllocatedAffected Insertnew
					 */
					aPrmaster = productService.getProducts(" WHERE prMasterID=" + aPrwhtransferdetail.getPrMasterId());
					BigDecimal qi = aPrwhtransferdetail.getQuantityTransfered().compareTo(new BigDecimal("0.0000")) == 1
							? aPrwhtransferdetail.getQuantityTransfered() : new BigDecimal("0.0000");
					theprwarehouse = (Prwarehouse) alogSession.get(Prwarehouse.class,
							thePrwarehouseTransfer.getPrFromWarehouseId());
					theprwarehsinventory = itsInventoryService.getPrwarehouseInventory(
					...
					prmatpInventoryLogMstr = new TpInventoryLogMaster(aPrmaster.getPrMasterId(),
							aPrmaster.getItemCode(), thePrwarehouseTransfer.getPrFromWarehouseId(),
							theprwarehouse.getSearchName(), aPrmaster.getInventoryOnHand(),
							theprwarehsinventory.getInventoryOnHand(), qi, BigDecimal.ZERO, "IT",
							thePrwarehouseTransfer.getPrTransferId(), aPrwhtransferdetail.getPrTransferDetailId(),
							thePrwarehouseTransfer.getTransactionNumber().toString(), null,
							/* Product out */(qi.compareTo(BigDecimal.ZERO) > 0) ? qi : BigDecimal.ZERO,
							/* Product in */(qi.compareTo(BigDecimal.ZERO) < 0) ? qi.multiply(new BigDecimal(-1))
									: BigDecimal.ZERO,
							"IT Edited", aPrwhtransferdetail.getUserId(), aPrwhtransferdetail.getUserName(),
							new java.util.Date());
					itsInventoryService.addTpInventoryLogMaster(prmatpInventoryLogMstr);
					...
					/*
					 * TpInventoryLogMaster Created on 04-12-2015 Code Starts
					 * ToWarehouseID OrderAffected Insertnew
					 */
					theprwarehouse = (Prwarehouse) alogSession.get(Prwarehouse.class,
							thePrwarehouseTransfer.getPrToWarehouseId());
					theprwarehsinventory = itsInventoryService.getPrwarehouseInventory(
					...
					prmatpInventoryLogMstr = new TpInventoryLogMaster(aPrmaster.getPrMasterId(),
							aPrmaster.getItemCode(), thePrwarehouseTransfer.getPrToWarehouseId(),
							theprwarehouse.getSearchName(), aPrmaster.getInventoryOnHand(),
							theprwarehsinventory.getInventoryOnHand(), BigDecimal.ZERO, qi, "IT",
							thePrwarehouseTransfer.getPrTransferId(), aPrwhtransferdetail.getPrTransferDetailId(),
							thePrwarehouseTransfer.getTransactionNumber().toString(), null,
							/* Product out */(qi.compareTo(BigDecimal.ZERO) < 0) ? qi.multiply(new BigDecimal(-1))
									: BigDecimal.ZERO,
							/* Product in */(qi.compareTo(BigDecimal.ZERO) > 0) ? qi : BigDecimal.ZERO, "IT Edited",
							aPrwhtransferdetail.getUserId(), aPrwhtransferdetail.getUserName(), new java.util.Date());
					itsInventoryService.addTpInventoryLogMaster(prmatpInventoryLogMstr);
					/* Code Ends */
	...
	public String updatePrMasterForWTransfer(Prwhtransferdetail objCusodetail, BigDecimal oldQuantityOrdered,
			BigDecimal newQuantityOrdered, String operation, Prwhtransferdetail oldobjCusodetail) throws JobException {
		...
		try {
			...
			if ("edit".equalsIgnoreCase(operation)) {
				if (objCusodetail.getPrMasterId().equals(oldobjCusodetail.getPrMasterId())) {
					...
					aTransaction.commit();
					aTransaction.begin();
					objPrmaster = (Prmaster) aVePOSession.get(Prmaster.class, objCusodetail.getPrMasterId());
			...
			itsLogger.error(e.getMessage(), e);
			JobException aJobException = new JobException(e.getCause().getMessage(), e);
			throw aJobException;
		} finally {
			aVePOSession.flush();
			aVePOSession.close();
	...
	/*
	 * public String updatePrWarehouseInventoryForWTransfer(Prwhtransferdetail
	 * objCusodetail,BigDecimal oldQuantityOrdered, BigDecimal
	 * newQuantityOrdered, String oper, Prwhtransferdetail oldobjCusodetail)
	 * throws JobException { Session aVePOSession =
	 * itsSessionFactory.openSession(); Transaction aTransaction; try {
	 * aTransaction = aVePOSession.beginTransaction(); aTransaction.begin();
	 * Cuso aCuso = (Cuso) aVePOSession.get(Cuso.class,
	 * objCusodetail.getCuSoid()); Integer sPrWarehouseID = 0; BigDecimal
	 * inventoryAllocated = new BigDecimal(0); Integer prWarehouseInventoryID =
	 * 0; String sQuery =
	 * "SELECT prFromWarehouseID FROM cuSO cuso WHERE cuSOID = "
	 * +aCuso.getCuSoid(); Query query = aVePOSession.createSQLQuery(sQuery);
	 * if(query.list().size() > 0) sPrWarehouseID =
	 * (Integer)query.list().get(0);
	 * 
	 * String sQuery1 =
	 * "SELECT inventory.InventoryAllocated,inventory.prWarehouseInventoryID FROM prWarehouseInventory inventory WHERE inventory.prMasterID = "
	 * +objCusodetail.getPrMasterId()+" AND prWarehouseID = "+sPrWarehouseID;
	 * query = aVePOSession.createSQLQuery(sQuery1); Iterator<?> iterator =
	 * query.list().iterator(); while(iterator.hasNext()) { Object[] object =
	 * (Object[]) iterator.next(); onOrder.add((BigDecimal)object[0]); }
	 * if(query.list().size() > 0){ Object[] object =
	 * (Object[])query.list().get(0); inventoryAllocated =
	 * (BigDecimal)object[0]; prWarehouseInventoryID = (Integer)object[1]; }
	 * Prwarehouseinventory objPrwarehouseinventory = (Prwarehouseinventory)
	 * aVePOSession.get(Prwarehouseinventory.class, prWarehouseInventoryID);
	 * BigDecimal allocated = objPrwarehouseinventory.getInventoryAllocated();
	 * if("del".equalsIgnoreCase(oper)){
	 * objPrwarehouseinventory.setInventoryAllocated(allocated.subtract(
	 * objCusodetail .getQuantityOrdered()));
	 * aVePOSession.update(objPrwarehouseinventory); aTransaction.commit(); }
	 * else if("add".equalsIgnoreCase(oper)){
	 * objPrwarehouseinventory.setInventoryAllocated(allocated.add(objCusodetail
	 * .getQuantityOrdered())); aVePOSession.update(objPrwarehouseinventory);
	 * aTransaction.commit(); } else if ("edit".equalsIgnoreCase(oper)) { if
	 * (oldobjCusodetail.getPrMasterId().equals( objCusodetail.getPrMasterId()))
	 * {
	 * 
	 * BigDecimal newAllocated =
	 * allocated.subtract(oldobjCusodetail.getQuantityOrdered()); BigDecimal
	 * newOnAllocated = newAllocated.add(objCusodetail.getQuantityOrdered());
	 ...
	 * 
	 * BigDecimal newAllocated =
	 * allocated.subtract(oldobjCusodetail.getQuantityOrdered());
	 * objPrwarehouseinventory.setInventoryAllocated(newAllocated);
	 * aVePOSession.update(objPrwarehouseinventory); aTransaction.commit();
	 * 
	 * 
	 * aTransaction.begin(); String sQuery2 =
	 * "SELECT inventory.InventoryAllocated,inventory.prWarehouseInventoryID FROM prWarehouseInventory inventory WHERE inventory.prMasterID = "
	 * +objCusodetail.getPrMasterId()+" AND prWarehouseID = "+sPrWarehouseID;
	 * Query query1 = aVePOSession.createSQLQuery(sQuery2);
	 * 
	 * if(query1.list().size() > 0){ Object[] object =
	 * (Object[])query1.list().get(0); inventoryAllocated =
	 * (BigDecimal)object[0]; prWarehouseInventoryID = (Integer)object[1]; }
	 * Prwarehouseinventory objPrwarehouseinventory1 = (Prwarehouseinventory)
	 * aVePOSession.get(Prwarehouseinventory.class, prWarehouseInventoryID);
	 ...
	 * aVePOSession.update(objPrwarehouseinventory1); aTransaction.commit(); }
	 * 
	 * }
	 * 
	 * 
	 ...
	 * objPrwarehouseinventory.getInventoryAllocated(); BigDecimal newValue =
	 * order.subtract(oldQuantityOrdered); BigDecimal allocatedNew =
	 * newValue.add(newQuantityOrdered);
	 * objPrwarehouseinventory.setInventoryAllocated(allocatedNew);
	 * 
	 * } aVePOSession.update(objPrwarehouseinventory); aTransaction.commit(); }
	 * catch (Exception e) { itsLogger.error(e.getMessage(), e); JobException
	 * aJobException = new JobException(e.getCause() .getMessage(), e); throw
	 * aJobException; } finally { aVePOSession.flush(); aVePOSession.close(); }
	 * return "Success"; }
	 */
	...
	@Override
	public String updatePrMasterForReceiveItemsWTransfer(Integer prTransferId, String receiveOrUndo, int UserID,
		...
		try {
			if ("del".equalsIgnoreCase(receiveOrUndo)) {
				...
				if ((createdDate != null && createdDate != "")) {
					aPrwarehouseTransfer
							.setReceivedDate(DateUtils.parseDate(createdDate, new String[] { "MM/dd/yyyy" }));
				}
			...
			List<Prwhtransferdetail> alPrwhtransferdetail = (List<Prwhtransferdetail>) aVePOSession
					.createCriteria(Prwhtransferdetail.class).add(Restrictions.eq("prTransferId", prTransferId)).list();
			if (alPrwhtransferdetail.size() > 0) {
				for (Prwhtransferdetail objPrwhtransferdetail : alPrwhtransferdetail) {
					// updatePrMasterForReceiveItemsWTransfer(objPrwhtransferdetail.getPrMasterId(),
					// objPrwhtransferdetail.getQuantityTransfered(),receiveOrUndo);
					updatePrWarehouseInventoryForWTransfer(aPrwarehouseTransfer.getPrFromWarehouseId(),
							aPrwarehouseTransfer.getPrToWarehouseId(), objPrwhtransferdetail.getPrMasterId(),
							objPrwhtransferdetail.getQuantityTransfered(), receiveOrUndo);
					updatePrMasterForWTransfer(objPrwhtransferdetail.getPrMasterId(),
							objPrwhtransferdetail.getQuantityTransfered(), receiveOrUndo);
					...
					if ("del".equalsIgnoreCase(receiveOrUndo)) {
						 ...
						 * Starts FromWarehouseID AllocatedAffected Rollback
						 */
						Session alogSession = itsSessionFactory.openSession();
						Prmaster aPrmaster = productService
								.getProducts(" WHERE prMasterID=" + objPrwhtransferdetail.getPrMasterId());
						BigDecimal qo = objPrwhtransferdetail.getQuantityTransfered()
								.compareTo(new BigDecimal("0.0000")) == 1
										? objPrwhtransferdetail.getQuantityTransfered() : new BigDecimal("0.0000");
						Prwarehouse theprwarehouse = (Prwarehouse) alogSession.get(Prwarehouse.class,
								aPrwarehouseTransfer.getPrFromWarehouseId());
						Prwarehouseinventory theprwarehsinventory = itsInventoryService.getPrwarehouseInventory(
								aPrwarehouseTransfer.getPrFromWarehouseId(), objPrwhtransferdetail.getPrMasterId());
						TpInventoryLogMaster prmatpInventoryLogMstr = new TpInventoryLogMaster(
								objPrwhtransferdetail.getPrMasterId(), aPrmaster.getItemCode(),
								aPrwarehouseTransfer.getPrFromWarehouseId(), theprwarehouse.getSearchName(),
								aPrmaster.getInventoryOnHand(), theprwarehsinventory.getInventoryOnHand(),
								qo.multiply(new BigDecimal(-1)), BigDecimal.ZERO, "IT",
								aPrwarehouseTransfer.getPrTransferId(), objPrwhtransferdetail.getPrTransferDetailId(),
								aPrwarehouseTransfer.getTransactionNumber().toString(), null,
								/* Product out */(qo.compareTo(BigDecimal.ZERO) < 0) ? qo.multiply(new BigDecimal(-1))
										: BigDecimal.ZERO,
								/* Product in */(qo.compareTo(BigDecimal.ZERO) > 0) ? qo : BigDecimal.ZERO,
								"IT ReceiveItems", UserID, username, new java.util.Date());
						itsInventoryService.addTpInventoryLogMaster(prmatpInventoryLogMstr);
						/* Code Ends */

						/*
						 * TpInventoryLogMaster Created on 04-12-2015 Code
						 * Starts ToWarehouseID OrderAffected Rollback
						 */
						theprwarehouse = (Prwarehouse) alogSession.get(Prwarehouse.class,
								aPrwarehouseTransfer.getPrToWarehouseId());
						theprwarehsinventory = itsInventoryService.getPrwarehouseInventory(
								aPrwarehouseTransfer.getPrToWarehouseId(), aPrmaster.getPrMasterId());
						prmatpInventoryLogMstr = new TpInventoryLogMaster(aPrmaster.getPrMasterId(),
								aPrmaster.getItemCode(), aPrwarehouseTransfer.getPrToWarehouseId(),
								theprwarehouse.getSearchName(), aPrmaster.getInventoryOnHand(),
								theprwarehsinventory.getInventoryOnHand(), BigDecimal.ZERO,
								qo.multiply(new BigDecimal(-1)), "IT", aPrwarehouseTransfer.getPrTransferId(),
								objPrwhtransferdetail.getPrTransferDetailId(),
								aPrwarehouseTransfer.getTransactionNumber().toString(), null,
								/* Product out */(qo.compareTo(BigDecimal.ZERO) > 0) ? qo : BigDecimal.ZERO,
								/* Product in */(qo.compareTo(BigDecimal.ZERO) < 0) ? qo.multiply(new BigDecimal(-1))
										: BigDecimal.ZERO,
						...
						TpInventoryLogMaster prmatpInventoryLogMstr = new TpInventoryLogMaster(
								objPrwhtransferdetail.getPrMasterId(), aPrmaster.getItemCode(),
								aPrwarehouseTransfer.getPrFromWarehouseId(), theprwarehouse.getSearchName(),
								aPrmaster.getInventoryOnHand(), theprwarehsinventory.getInventoryOnHand(), qo,
								BigDecimal.ZERO, "IT", aPrwarehouseTransfer.getPrTransferId(),
								objPrwhtransferdetail.getPrTransferDetailId(),
								aPrwarehouseTransfer.getTransactionNumber().toString(), null,
								/* Product out */(qo.compareTo(BigDecimal.ZERO) > 0) ? qo : BigDecimal.ZERO,
								/* Product in */(qo.compareTo(BigDecimal.ZERO) < 0) ? qo.multiply(new BigDecimal(-1))
										: BigDecimal.ZERO,
								"IT UNDO", UserID, username, new java.util.Date());
						itsInventoryService.addTpInventoryLogMaster(prmatpInventoryLogMstr);
						/* Code Ends */
						...
						prmatpInventoryLogMstr = new TpInventoryLogMaster(aPrmaster.getPrMasterId(),
								...
								/* Product out */(qo.compareTo(BigDecimal.ZERO) < 0) ? qo.multiply(new BigDecimal(-1))
										: BigDecimal.ZERO,
								/* Product in */(qo.compareTo(BigDecimal.ZERO) > 0) ? qo : BigDecimal.ZERO, "IT UNDO",
								UserID, username, new java.util.Date());
						itsInventoryService.addTpInventoryLogMaster(prmatpInventoryLogMstr);
						/* Code Ends */
					}
				}
	...
	public BigDecimal amountCalculation(Prwhtransferdetail objPrwhtransferdetail) {
		...
		try {
			...
			BigDecimal prcost = objPrmaster.getAverageCost() == null ? new BigDecimal(0.0000)
					: objPrmaster.getAverageCost();
	...
	public void AddGeneralLedgerProcess(PrwarehouseTransfer aPrwarehouseTransfer, BigDecimal totAmount, String username)
		...
		try {
			...
			fromPrwarehouse = (Prwarehouse) aopenSession.get(Prwarehouse.class,
					aPrwarehouseTransfer.getPrFromWarehouseId());
			toPrwarehouse = (Prwarehouse) aopenSession.get(Prwarehouse.class,
					aPrwarehouseTransfer.getPrToWarehouseId());

			Coaccount fromCoAccountIdapdetails = callgltransaction
					.getCoaccountDetailsBasedoncoAccountid(fromPrwarehouse.getCoAccountIdasset());
			Coaccount toCoAccountIdapdetails = callgltransaction
					.getCoaccountDetailsBasedoncoAccountid(toPrwarehouse.getCoAccountIdasset());
			...
			/*
			 * aTransaction = aopenSession.beginTransaction(); Prmaster
			 * objPrmaster = (Prmaster) aopenSession.get(Prmaster.class,
			 * objPrwhtransferdetail.getPrMasterId()); BigDecimal
			 * prcost=objPrmaster.getLastCost(); BigDecimal
			 * prmul=objPrmaster.getPOMult(); BigDecimal
			 * prQty=objPrwhtransferdetail.getQuantityTransfered(); BigDecimal
			 * TotalCost=prcost.multiply(prQty); if(prmul!=null &&
			 * prmul.compareTo(BigDecimal.ZERO)!=0){
			 * TotalCost=TotalCost.multiply(prmul); }
			 */
	...
	public String updatePrMasterForWTransfer(Integer prMasterID, BigDecimal qtyTransfered, String operation)
			throws JobException {
		Session aVePOSession = itsSessionFactory.openSession();
		Transaction aTransaction;
		...
		try {
			...
			if ("add".equalsIgnoreCase(operation)) {
				Prmaster objPrmaster = (Prmaster) aVePOSession.get(Prmaster.class, prMasterID);
				BigDecimal allocated = objPrmaster.getInventoryAllocated() != null ? objPrmaster.getInventoryAllocated()
						: BigDecimal.ZERO;
				objPrmaster.setInventoryAllocated(allocated.add(qtyTransfered));
				BigDecimal onorder = objPrmaster.getInventoryOnOrder() != null ? objPrmaster.getInventoryOnOrder()
						: BigDecimal.ZERO;
	...
	public String updatePrMasterForReceiveItemsWTransfer(Integer prMasterID, BigDecimal qtyTransfered, String operation)
			throws JobException {
		Session aVePOSession = itsSessionFactory.openSession();
		Transaction aTransaction;
		try {
	...
	public String updatePrWarehouseInventoryForWTransfer(Integer fromWarehouse, Integer toWarehouse, Integer prMasterID,
			BigDecimal qty, String oper) throws JobException {
		Session aVePOSession = itsSessionFactory.openSession();
		Transaction aTransaction;
		String sQuery = "";
		try {
			...
			if ("add".equalsIgnoreCase(oper)) {
				sQuery = "SELECT prWarehouseInventoryID FROM prWarehouseInventory WHERE prMasterID = " + prMasterID
						+ " AND prWarehouseID = " + fromWarehouse;
				Query query = aVePOSession.createSQLQuery(sQuery);
				if (query.list().size() > 0)
					sPrWarehouseID = (Integer) query.list().get(0);

				Prwarehouseinventory aaPrwarehouseinventory = (Prwarehouseinventory) aVePOSession
						.get(Prwarehouseinventory.class, sPrWarehouseID);
				...
				aTransaction.begin();
				sQuery = "SELECT prWarehouseInventoryID FROM prWarehouseInventory WHERE prMasterID = " + prMasterID
						+ " AND prWarehouseID = " + toWarehouse;
				query = aVePOSession.createSQLQuery(sQuery);
				if (query.list().size() > 0)
					sPrWarehouseID = (Integer) query.list().get(0);

				aaPrwarehouseinventory = (Prwarehouseinventory) aVePOSession.get(Prwarehouseinventory.class,
						sPrWarehouseID);
				...
				BigDecimal onHand = aaPrwarehouseinventory.getInventoryOnHand();
				aaPrwarehouseinventory.setInventoryOnHand(onHand.subtract(qty));
				aaPrwarehouseinventory
						.setInventoryAllocated(aaPrwarehouseinventory.getInventoryAllocated().subtract(qty));
				aTransaction.commit();
				aTransaction.begin();
				sQuery = "SELECT prWarehouseInventoryID FROM prWarehouseInventory WHERE prMasterID = " + prMasterID
						+ " AND prWarehouseID = " + toWarehouse;
	...
	/*
	 * @Override public PrwarehouseTransfer
	 * copyTransferLineDetails(PrwarehouseTransfer aPrwarehouseTransfer) throws
	 * InventoryException { Session aPrWarehouseSession =
	 * itsSessionFactory.openSession(); PrwarehouseTransfer
	 * objPrwarehouseTransfer = new PrwarehouseTransfer(); Transaction
	 * aTransaction; try { aTransaction =
	 * aPrWarehouseSession.beginTransaction(); aTransaction.begin(); Integer id
	 * = (Integer) aPrWarehouseSession.save(aPrwarehouseTransfer);
	 * System.out.println("saveTransferDetails---->"+id); objPrwarehouseTransfer
	 * = (PrwarehouseTransfer)
	 * aPrWarehouseSession.get(PrwarehouseTransfer.class, id);
	 * aTransaction.commit(); } catch (Exception excep) {
	 * itsLogger.error(excep.getMessage(), excep); throw new
	 * InventoryException(excep.getMessage(), excep); } finally {
	 * aPrWarehouseSession.flush(); aPrWarehouseSession.close(); } return
	 * objPrwarehouseTransfer; }
	 */
	@Override
	public String copyTransferLineDetails(Integer prTransferID, Integer newPrTransferID) throws JobException {
		...
		try {
			...
			// aSession.get(Prwhtransferdetail.class, prTransferID);
			List<Prwhtransferdetail> alPrwhtransferdetail = (List<Prwhtransferdetail>) aSession
					.createCriteria(Prwhtransferdetail.class).add(Restrictions.eq("prTransferId", prTransferID)).list();
			if (alPrwhtransferdetail.size() > 0) {
				for (Prwhtransferdetail aPrwhtransferdetail : alPrwhtransferdetail) {
					 ...
					 * Stringbuffer // creates less // no of // objects.
					 * "INSERT prWHtransferDetail") .append(
					 * "(prTransferID,prMasterID, Description,") .append(
					 * "QuantityTransfered, ").append("InventoryCost, ")
					 * .append("QuantityAvailable, ").append(" ItemCode "
					 * ).append(") ").append("SELECT  ")
					 * .append(newPrTransferID).append(",prMasterID") .append(
					 * " ,Description ").append(" ,QuantityTransfered ")
					 * .append(",InventoryCost ").append(",QuantityAvailable ")
					 * .append(",ItemCode ").append("FROM prWHtransferDetail ")
					 * .append("WHERE prTransferID = "
					 * ).append(prTransferID).append(";"); aInsertInvoiceQuery =
					 * insertInvoiceQuery.toString();
					 * aSession.createSQLQuery(aInsertInvoiceQuery).
					 * executeUpdate();
					 */
					aTransaction.commit();
					aPrwarehouseTransfer = (PrwarehouseTransfer) aSession.get(PrwarehouseTransfer.class,
							newPrTransferID);
					Prwhtransferdetail bPrwhtransferdetail = (Prwhtransferdetail) aSession.get(Prwhtransferdetail.class,
							newPrTransferDetailID);
	...
	public int getInventoryRecordCount(String itemCode) throws InventoryException {
		...
		if (itemCode != null)
			aStr = aStr + " FROM prMaster WHERE Description NOT LIKE '%DO NOT %' AND ItemCode LIKE '%" + itemCode
					+ "%' ORDER BY prMaster.ItemCode ";
		else
	...
	public int getInventoryRecordCount(Integer warehouseId, String theSearchString, Integer inactive, String column,
			String sortBy) throws InventoryException {
		Session aSession = null;
		BigInteger aTotalCount = null;
		String itemcodeCondn = "";
		/*
		...
		/*
		 * if(column.equalsIgnoreCase("description")){ sortByOption=
		 * " pr.Description "; }else
		 * if(column.equalsIgnoreCase("prDepartment")){ sortByOption=
		 * " pd.Description "; }else if(column.equalsIgnoreCase("prCategory")){
		 * sortByOption=" pc.Description "; }else
		 * if(column.equalsIgnoreCase("vendorName")){ sortByOption=" rx.Name ";
		 * }else if(column.equalsIgnoreCase("itemCode")){ sortByOption=
		 * " pr.ItemCode "; }else
		 * if(column.equalsIgnoreCase("inventoryOnHand")){ sortByOption=
		 * " pw.InventoryOnHand "; }else
		 * if(column.equalsIgnoreCase("lastCost")){ sortByOption=" pr.LastCost "
		 * ; }else if(column.equalsIgnoreCase("totalCost")){ sortByOption=
		 * " TotalCost "; }
		 */
		...
		String aStr = "SELECT COUNT(DISTINCT(pr.prMasterId)) FROM prMaster pr LEFT JOIN prWarehouseInventory pw ON pr.prMasterId = pw.prMasterID LEFT JOIN prDepartment pd ON pd.prDepartmentID=pr.prDepartmentID LEFT JOIN prCategory pc ON pc.prCategoryID = pr.prCategoryID LEFT JOIN rxMaster rx ON rx.rxMasterID = pr.rxMasterIDPrimaryVendor WHERE "
				+ warehouseCondn + " " + inactiveCondn
				+ itemcodeCondn/*
								 * +" ORDER BY "
								 * +sortByOption+sortBy.toUpperCase()+" "
								 */;
	...
	/**
	 * Created by : Praveenkumar Date : 2014-09-19 Purpose: Calculate warehouse
	 * cost replicated by turbo rep app
	 * 
	 */
	@Override
	public BigDecimal getWarehouseCost(Integer prMasterID) throws JobException {
		...
		try {
			...
			Prmaster aPrmaster = new Prmaster();
			if (prMasterID != null)
				aPrmaster = (Prmaster) aSession.get(Prmaster.class, prMasterID);
			...
			// BigDecimal avgcost = aPrmaster.getLastCost().setScale(2,
			// BigDecimal.ROUND_DOWN).multiply(aPrmaster.getPOMult().setScale(2,
			// BigDecimal.ROUND_DOWN));
			BigDecimal avgcost = aPrmaster.getAverageCost() == null
					? new BigDecimal("0.0000").setScale(2, BigDecimal.ROUND_FLOOR)
					: aPrmaster.getAverageCost().setScale(2, BigDecimal.ROUND_FLOOR);// .multiply(aPrmaster.getPOMult().setScale(2,
																						// BigDecimal.ROUND_DOWN));
			...
			// aPrmaster.get
			aTransaction.commit();
			itsLogger.info("WareHouseCost::" + warehouseCost + " MarkupAmount::" + inventoryMarkup + " AverageCost::"
					+ avgcost);
	...
	public boolean saveInventorysettings(ArrayList<Sysvariable> sysvariablelist) throws InventoryException {
		...
		try {
			...
			for (Sysvariable listSysvariable : sysvariablelist) {
				...
				aTransaction = aPrWarehouseSession.beginTransaction();
				aTransaction.begin();
				aSysvariable = (Sysvariable) aPrWarehouseSession.get(Sysvariable.class,
						listSysvariable.getSysVariableId());
	...
	public PrCategory getprCategories(Integer prcategoryId) throws InventoryException {
		String aDeptAccSelectQuery = "SELECT prCategoryID,Description,InActive,MarkupCost,MarkupAmount FROM prCategory WHERE prCategoryID="
				+ prcategoryId + " And MarkupCost=1";
		Session aSession = null;
	...
	public String updatePrMasterAndPrwhInventory(PrwarehouseTransfer aPrwarehouseTransfer,
			Prwhtransferdetail aPrwhtransferdetail, String oper) throws InventoryException {
		...
		try {
			...
			Prwhtransferdetail oldPrwhtransferdetail = (Prwhtransferdetail) aPrWarehouseSession
					.get(Prwhtransferdetail.class, aPrwhtransferdetail.getPrTransferDetailId());

			Query aQuery = aPrWarehouseSession.createQuery(
					"FROM  Prwarehouseinventory where prWarehouseId=" + aPrwarehouseTransfer.getPrFromWarehouseId()
							+ " and prMasterId=" + aPrwhtransferdetail.getPrMasterId());
			aQueryList = aQuery.list();
			if (aQueryList != null) {
				PrwarehouseinventoryfromId = aQueryList.get(0).getPrWarehouseInventoryId();
			}
			aQueryList = null;
			aQuery = aPrWarehouseSession.createQuery(
					"FROM  Prwarehouseinventory where prWarehouseId=" + aPrwarehouseTransfer.getPrToWarehouseId()
							+ " and prMasterId=" + aPrwhtransferdetail.getPrMasterId());
			aQueryList = aQuery.list();
			if (aQueryList != null) {
				PrwarehouseinventorytoId = aQueryList.get(0).getPrWarehouseInventoryId();
			}
			Prwarehouseinventory objfrmprwarehouseinventory = (Prwarehouseinventory) aPrWarehouseSession
					.get(Prwarehouseinventory.class, PrwarehouseinventoryfromId);
			Prwarehouseinventory objtoprwarehouseinventory = (Prwarehouseinventory) aPrWarehouseSession
					.get(Prwarehouseinventory.class, PrwarehouseinventorytoId);
			if (oper.equals("add")) {
				/*
				 * objfrmprwarehouseinventory.setInventoryOnHand(
				 * objfrmprwarehouseinventory.getInventoryOnHand()==null?(
				 * BigDecimal.ZERO).subtract(aPrwhtransferdetail.
				 * getQuantityTransfered()):objfrmprwarehouseinventory.
				 * getInventoryOnHand().subtract(aPrwhtransferdetail.
				 * getQuantityTransfered()));
				 * objtoprwarehouseinventory.setInventoryOnHand(
				 * objtoprwarehouseinventory.getInventoryOnHand()==null?(
				 * BigDecimal.ZERO).add(aPrwhtransferdetail.
				 * getQuantityTransfered()):objtoprwarehouseinventory.
				 * getInventoryOnHand().add(aPrwhtransferdetail.
				 * getQuantityTransfered()));
				 */
				objfrmprwarehouseinventory
						.setInventoryAllocated(objfrmprwarehouseinventory.getInventoryAllocated() == null
								? (BigDecimal.ZERO).add(aPrwhtransferdetail.getQuantityTransfered())
								: objfrmprwarehouseinventory.getInventoryAllocated()
										.add(aPrwhtransferdetail.getQuantityTransfered()));
				objtoprwarehouseinventory.setInventoryOnOrder(objtoprwarehouseinventory.getInventoryOnOrder() == null
						? (BigDecimal.ZERO).add(aPrwhtransferdetail.getQuantityTransfered())
						: objtoprwarehouseinventory.getInventoryOnOrder()
								.add(aPrwhtransferdetail.getQuantityTransfered()));
				aPrWarehouseSession.update(objfrmprwarehouseinventory);
				aPrWarehouseSession.update(objtoprwarehouseinventory);

				updatePrMasterForWTransfer(oldPrwhtransferdetail.getPrMasterId(),
						oldPrwhtransferdetail.getQuantityTransfered(), oper);
				// updatePrMasterForReceiveItemsWTransfer(objPrwhtransferdetail.getPrMasterId(),
				// objPrwhtransferdetail.getQuantityTransfered(),receiveOrUndo)
				 ...
				 * objfrmprwarehouseinventory.setInventoryOnHand(
				 * objfrmprwarehouseinventory.getInventoryOnHand()==null?(
				 * BigDecimal.ZERO).add(oldPrwhtransferdetail.
				 * getQuantityTransfered()).subtract(aPrwhtransferdetail.
				 * getQuantityTransfered()):
				 * objfrmprwarehouseinventory.getInventoryOnHand().add(
				 * oldPrwhtransferdetail.getQuantityTransfered()).subtract(
				 * aPrwhtransferdetail.getQuantityTransfered()));
				 * objtoprwarehouseinventory.setInventoryOnHand(
				 * objtoprwarehouseinventory.getInventoryOnHand()==null?(
				 * BigDecimal.ZERO).subtract(oldPrwhtransferdetail.
				 * getQuantityTransfered()).add(aPrwhtransferdetail.
				 * getQuantityTransfered()):
				 * objtoprwarehouseinventory.getInventoryOnHand().subtract(
				 * oldPrwhtransferdetail.getQuantityTransfered()).add(
				 * aPrwhtransferdetail.getQuantityTransfered()));
				 */

				objfrmprwarehouseinventory
						.setInventoryAllocated(objfrmprwarehouseinventory.getInventoryAllocated() == null
								? (BigDecimal.ZERO).subtract(oldPrwhtransferdetail.getQuantityTransfered())
										.add(aPrwhtransferdetail.getQuantityTransfered())
								: objfrmprwarehouseinventory.getInventoryAllocated()
										.subtract(oldPrwhtransferdetail.getQuantityTransfered())
										.add(aPrwhtransferdetail.getQuantityTransfered()));
				objtoprwarehouseinventory.setInventoryOnOrder(objtoprwarehouseinventory.getInventoryOnOrder() == null
						? (BigDecimal.ZERO).subtract(oldPrwhtransferdetail.getQuantityTransfered())
								.add(aPrwhtransferdetail.getQuantityTransfered())
				...
				aPrWarehouseSession.update(objtoprwarehouseinventory);
				updatePrMasterForWTransfer(oldPrwhtransferdetail.getPrMasterId(),
						oldPrwhtransferdetail.getQuantityTransfered(), "del");
				updatePrMasterForWTransfer(oldPrwhtransferdetail.getPrMasterId(),
						aPrwhtransferdetail.getQuantityTransfered(), "add");
				 ...
				 * BigDecimal.ZERO).add(oldPrwhtransferdetail.
				 * getQuantityTransfered()):objfrmprwarehouseinventory.
				 * getInventoryOnHand().add(oldPrwhtransferdetail.
				 * getQuantityTransfered()));
				 * objtoprwarehouseinventory.setInventoryOnHand(
				 * objtoprwarehouseinventory.getInventoryOnHand()==null?(
				 * BigDecimal.ZERO).subtract(oldPrwhtransferdetail.
				 * getQuantityTransfered()):objtoprwarehouseinventory.
				 * getInventoryOnHand().subtract(oldPrwhtransferdetail.
				 * getQuantityTransfered()));
				 */
				...
				aPrWarehouseSession.update(objfrmprwarehouseinventory);
				aPrWarehouseSession.update(objtoprwarehouseinventory);
				updatePrMasterForWTransfer(objfrmprwarehouseinventory.getPrMasterId(),
						oldPrwhtransferdetail.getQuantityTransfered(), "del");
			}
	...
	public Integer getRecordsCountforReceiveInvenory(int warehouseListID, String fromDateID, String toDateID,
			int prMasterID) {
		// String aJobCountStr = "SELECT COUNT(vePOID) AS count FROM vePO";

		// String aJobCountStr = "SELECT COUNT(vp.vePOID) AS COUNT FROM vePO vp
		// WHERE receiveddate <>''";

		String aJobCountStr = "SELECT  COUNT(vePO.vePOID) AS COUNT FROM vePO "
				+ " LEFT JOIN vePODetail ON vePO.vePOID=vePODetail.vePOID WHERE vePODetail.QuantityReceived IS NOT NULL AND vePODetail.prMasterID='"
				+ prMasterID + "' AND vePO.CreatedOn BETWEEN '" + fromDateID + " 00:00:00' AND '" + toDateID
				+ " 23:59:59'  ";

		if (warehouseListID > 0)
			aJobCountStr = aJobCountStr + " AND vePO.prWarehouseID='" + warehouseListID + "'";

		// System.out.println("PurchaseServiceImpl Integer getRecordsCount()
		// query string :: "+aJobCountStr);
		Session aSession = null;
		BigInteger aTotalCount = null;
		try {
	...
	@Override
	public List<PurchaseOrdersBean> filterInventoryTransactionListSize(String startDate, String endDate,
			int warehouseListID, int prMasterID) throws VendorException, ParseException {
		...
		StringBuilder aPOSelectQry = new StringBuilder("SELECT H.prTransferID AS Id, H.TransferDate AS TranDate,"
				...
				+ "FROM prTransfer AS H RIGHT OUTER JOIN prTransferDetail AS D ON H.prTransferID = D.prTransferID WHERE D.prMasterID = "
				+ prMasterID + " ");

		if (!startDate.equals("") || !endDate.equals("")) {
			aPOSelectQry
					.append(" AND H.TransferDate BETWEEN '" + startDate + " 00:00:00' AND '" + endDate + " 23:59:59'");
		}
		if (warehouseListID > 0) {
			aPOSelectQry.append(" AND (H.prFromWarehouseID = '" + warehouseListID + "' OR H.prToWarehouseID = '"
					+ warehouseListID + "')");
		}
		aPOSelectQry.append(" UNION ALL ");
		aPOSelectQry
				.append(" SELECT P.vePOID AS Id ,R.ReceiveDate AS TranDate, CAST(P.PONumber AS CHAR(50)) AS TransNumber,"
						...
						+ "ON R.veReceiveID = D.veReceiveID LEFT OUTER JOIN vePO AS P ON R.vePOID = P.vePOID WHERE D.prMasterID = "
						+ prMasterID + " ");
		if (!startDate.equals("") || !endDate.equals("")) {
		...
		aPOSelectQry.append(" UNION ALL ");
		aPOSelectQry
				.append(" SELECT I.cuInvoiceID as Id,I.InvoiceDate AS TranDate, CAST(I.InvoiceNumber AS CHAR(50)) AS TransNumber,"
						...
						+ "(I.TransactionStatus > 0) AND (I.prFromWarehouseID IS NOT NULL) AND D.prMasterID = "
						+ prMasterID + " ");
		...
		try {
			// Retrieve aSession from Hibernate
			aSession = itsSessionFactory.openSession();
			// System.out.println("Session statics: " +
			// itsSessionFactory.getStatistics().getConnectCount());
	...
	@Override
	public List<PurchaseOrdersBean> filterInventoryTransactionList(String startDate, String endDate,
			int warehouseListID, int prMasterID, String sortIndex, String sortOrder, int from, int to)
		 ...
		 * +
		 * "WHERE vePODetail.QuantityReceived IS NOT NULL AND vePODetail.prMasterID='"
		 * +prMasterID+"'");
		 */
		/*
		 * StringBuilder aPOSelectQry = new StringBuilder(
		 * "SELECT H.prTransferID AS Id, H.TransferDate AS TranDate," +
		 * "CAST(H.Description AS CHAR(50)) AS TransNumber, 'Inventory Transfer' AS TranDesc,"
		 * +
		 ...
		 * +prMasterID+" ");
		 * 
		 * if(warehouseListID>0){ aPOSelectQry.append(
		 * " AND (H.prFromWarehouseID = '"+warehouseListID+
		 * "' OR H.prToWarehouseID = '"+warehouseListID+"')"); }
		 * 
		 * if(!startDate.equals("") || !endDate.equals("")){
		 * aPOSelectQry.append(" AND H.TransferDate BETWEEN '"+startDate +
		...
		aPOSelectQry.append(" UNION ALL");
		aPOSelectQry
				.append(" SELECT  prWHtransfer.prTransferID AS Id,prWHtransfer.TransferDate AS TranDate,CASE prWHtransfer.screenno WHEN 2 THEN prWHtransfer.Description ELSE CAST(prWHtransfer.TransNumber AS CHAR(50)) END AS TransNumber,'Adjustments' AS Trans, "
						...
						+ " LEFT JOIN prMaster ON prWHtransferDetail.prMasterID = prMaster.prMasterID WHERE prWHtransferDetail.prMasterID="
						+ prMasterID + " ");
		if (warehouseListID > 0) {
		...
		if (!startDate.equals("") || !endDate.equals("")) {
			aPOSelectQry.append(" AND prWHtransfer.TransferDate BETWEEN '" + startDate + " 00:00:00' AND '" + endDate
					+ " 23:59:59'");
		}
		aPOSelectQry.append(" AND screenno IS NOT NULL ");

		/*
		 * aPOSelectQry.append(" UNION ALL "); aPOSelectQry.append(
		 * " SELECT  prWHtransfer.prTransferID AS Id,prWHtransfer.TransferDate AS TranDate,"
		 * +
		 ...
		 * + "prWHtransferDetail.prMasterID="+prMasterID+
		 * " AND prWHtransfer.prFromWarehouseID IS NOT NULL AND prWHtransfer.prToWarehouseID IS NOT NULL"
		 * ); if(!startDate.equals("") || !endDate.equals("")){
		 * aPOSelectQry.append(" AND prWHtransfer.TransferDate BETWEEN '"
		 * +startDate +" 00:00:00' AND '"+endDate+" 23:59:59'"); }
		 * if(warehouseListID>0){ aPOSelectQry.append(
		 * " AND (prWHtransfer.prFromWarehouseID='"+warehouseListID+"')"); }
		 * aPOSelectQry.append(" UNION ALL"); aPOSelectQry.append(
		 * " SELECT  prWHtransfer.prTransferID AS Id,prWHtransfer.TransferDate AS TranDate,CAST(prWHtransfer.TransNumber AS CHAR(50)) AS TransNumber,'Adjustments' AS Trans, "
		 * +
		 ...
		 * 
		 * if(warehouseListID>0){ aPOSelectQry.append(
		 * " AND (prWHtransfer.prToWarehouseID = '"+warehouseListID+"')"); }
		 * if(!startDate.equals("") || !endDate.equals("")){
		 * aPOSelectQry.append(" AND prWHtransfer.TransferDate BETWEEN '"
		 * +startDate +" 00:00:00' AND '"+endDate+" 23:59:59'"); }
		 * aPOSelectQry.append(" UNION ALL"); aPOSelectQry.append(
		 ...
		 * 
		 * aPOSelectQry.append(" UNION ALL"); aPOSelectQry.append(
		 * " SELECT  prWHtransfer.prTransferID AS Id, prWHtransfer.TransferDate AS TranDate,'Count' AS TransNumber,"
		 * +
		...
		try {
			...
			while (aIterator.hasNext()) {
				...
				/*
				 * if(screenno==0){ //Old Transfer Data
				 * if(((String)aObj[3]).equals("Inventory Transfer")){
				 * if(((BigInteger)aObj[5]).compareTo(new BigInteger("0"))!=0){
				 * 
				 * if(comparedVal.compareTo(new BigInteger("0"))!=0 &&
				 * comparedVal.compareTo((BigInteger)aObj[5])!=0){
				 * aPurchaseOrdersBean.setJobName("Inventory Transfer:"
				 * +(String)aObj[2]);
				 * aPurchaseOrdersBean.setDifference((BigDecimal) aObj[6]);
				 * runningBalance = runningBalance.subtract((BigDecimal)
				 * aObj[6]); }else{ aPurchaseOrdersBean.setJobName(
				 * "Inventory Transfer:"+(String)aObj[2]);
				 * aPurchaseOrdersBean.setQuantityOrdered((BigDecimal) aObj[6]);
				 ...
				 * aObj[6]); }
				 * 
				 * } }else{ if(((BigDecimal) aObj[6]).compareTo(new
				 * BigDecimal(0))==-1){ aPurchaseOrdersBean.setJobName(
				 * "Inventory Transfer:"+(String)aObj[2]);
				 * aPurchaseOrdersBean.setQuantityOrdered((BigDecimal) aObj[6]);
				 * runningBalance = runningBalance.add(((BigDecimal)
				 * aObj[6]).multiply(new BigDecimal(-1.0))); }else{
				 * aPurchaseOrdersBean.setJobName("Inventory Transfer:"
				 * +(String)aObj[2]); runningBalance =
				 * runningBalance.subtract((BigDecimal) aObj[6]);
				 * aPurchaseOrdersBean.setDifference((BigDecimal) aObj[6]); } }
				 ...
				 * aPurchaseOrdersBean.setQuantityOrdered((BigDecimal) aObj[6]);
				 * runningBalance = runningBalance.add((BigDecimal) aObj[6]);
				 * }else{ aPurchaseOrdersBean.setJobName("Inventory Transfer:"
				 * +(String)aObj[2]); runningBalance =
				 * runningBalance.subtract(((BigDecimal) aObj[6]).multiply(new
				 * BigDecimal(-1.0)));
				 * aPurchaseOrdersBean.setDifference(((BigDecimal)
				 * aObj[6]).multiply(new BigDecimal(-1.0))); } }else
				 * if(((String)aObj[3]).equals("Count")){ if(((BigDecimal)
				 * aObj[6]).compareTo(new BigDecimal(0))==1){
				 * aPurchaseOrdersBean.setJobName("Adjustment");
				 * aPurchaseOrdersBean.setDifference((BigDecimal) aObj[6]);
				 * runningBalance = runningBalance.subtract((BigDecimal)
				 * aObj[6]); }else{
				 * aPurchaseOrdersBean.setJobName((String)aObj[2]);
				 * aPurchaseOrdersBean.setQuantityOrdered(((BigDecimal)
				 * aObj[6]).multiply(new BigDecimal(-1.0))); runningBalance =
				 * runningBalance.add(((BigDecimal) aObj[6]).multiply(new
				 * BigDecimal(-1.0)));
				 * 
				 * } }
				 * 
				...
				if (screenno == -2) {
					...
					if (!(warehouseListID > 0) || (frID != 0 && warehouseListID == frID)) {
						...
						if (((BigDecimal) aObj[6]).compareTo(new BigDecimal(0)) > 0) {
							...
							fromPurchaseOrdersBean.setJobName("Adjustment:" + (String) aObj[2]);
							fromPurchaseOrdersBean
									.setQuantityOrdered(((BigDecimal) aObj[6]).multiply(new BigDecimal(-1.0)));
					...
					if (screenno == 1 && aObj[10] != null) {
						...
						if (!(warehouseListID > 0) || (toID != 0 && warehouseListID == toID)) {
							...
							if (((BigDecimal) aObj[6]).compareTo(new BigDecimal(0)) > 0) {
								...
								toPurchaseOrdersBean.setJobName("Inventory Transfer:" + (String) aObj[2]);
								toPurchaseOrdersBean
										.setDifference(((BigDecimal) aObj[6]).multiply(new BigDecimal(-1.0)));
						...
						// Custome Invoice Out
						if ((((BigDecimal) aObj[6]) == null ? new BigDecimal(0) : (BigDecimal) aObj[6])
							...
							runningBalance = runningBalance
									.subtract((BigDecimal) aObj[6] == null ? new BigDecimal(0) : (BigDecimal) aObj[6]);
						}
						aPurchaseOrdersBean.setInventoryOnHand(runningBalance);
						aQueryList.add(aPurchaseOrdersBean);
					}
			...
			/*
			 * while (aIterator.hasNext()) { aPurchaseOrdersBean = new
			 * PurchaseOrdersBean(); Object[] aObj = (Object[])aIterator.next();
			 * aPurchaseOrdersBean.setVePOID((Integer) aObj[0]); if(aObj[1] !=
			 * null) { Timestamp stamp = (Timestamp)aObj[1]; Date date = new
			 * Date(stamp.getTime()); aPurchaseOrdersBean.setCreatedOn(date);
			 * //(Date) aObj[1] } aPurchaseOrdersBean.setPONumber((String)
			 * aObj[2]);
			 * 
			 * 
			 * if((BigDecimal) aObj[8]!=null ) pricemul =(BigDecimal) aObj[8];
			 * if(pricemul.compareTo(BigDecimal.ZERO) > 0 && (String)aObj[3]==
			 * "Recieved PO"){ aPurchaseOrdersBean.setSubtotal(((BigDecimal)
			 * aObj[7]).multiply(pricemul)); } else{
			 * aPurchaseOrdersBean.setSubtotal((BigDecimal) aObj[7]); }
			 * if(aObj[6]!=null){ if(((String)aObj[3]).equals("Customer Invoice"
			 * )){ if((((BigDecimal) aObj[6])==null?new
			 * BigDecimal(0):(BigDecimal) aObj[6]).compareTo(new
			 * BigDecimal(0))==-1){ aPurchaseOrdersBean.setJobName("Invoices In"
			 * ); aPurchaseOrdersBean.setQuantityOrdered((BigDecimal) aObj[6]);
			 * runningBalance = runningBalance.add(((BigDecimal)
			 * aObj[6]).multiply(new BigDecimal(-1.0))); }else{
			 * aPurchaseOrdersBean.setJobName("Invoices Out");
			 * aPurchaseOrdersBean.setDifference((BigDecimal) aObj[6]);
			 * runningBalance = runningBalance.subtract((BigDecimal)
			 * aObj[6]==null?new BigDecimal(0):(BigDecimal) aObj[6]); }
			 * 
			 ...
			 * comparedVal.compareTo((BigInteger)aObj[5])!=0){
			 * aPurchaseOrdersBean.setJobName("Inventory Transfer:"
			 * +(String)aObj[2]); aPurchaseOrdersBean.setDifference((BigDecimal)
			 * aObj[6]); runningBalance = runningBalance.subtract((BigDecimal)
			 * aObj[6]); }else{ aPurchaseOrdersBean.setJobName(
			 * "Inventory Transfer:"+(String)aObj[2]);
			 * aPurchaseOrdersBean.setQuantityOrdered((BigDecimal) aObj[6]);
			 * runningBalance = runningBalance.add((BigDecimal) aObj[6]);
			 ...
			 * +(String)aObj[2]);
			 * aPurchaseOrdersBean.setQuantityOrdered((BigDecimal) aObj[6]);
			 * runningBalance = runningBalance.add((BigDecimal) aObj[6]); }else{
			 * aPurchaseOrdersBean.setJobName("Inventory Transfer:"
			 * +(String)aObj[2]); runningBalance =
			 * runningBalance.subtract(((BigDecimal) aObj[6]).multiply(new
			 * BigDecimal(-1.0)));
			 ...
			 * aPurchaseOrdersBean.setDifference((BigDecimal) aObj[6]);
			 * runningBalance = runningBalance.subtract((BigDecimal) aObj[6]);
			 * }else{ aPurchaseOrdersBean.setJobName((String)aObj[2]);
			 * aPurchaseOrdersBean.setQuantityOrdered(((BigDecimal)
			 * aObj[6]).multiply(new BigDecimal(-1.0))); runningBalance =
			 * runningBalance.add(((BigDecimal) aObj[6]).multiply(new
			 * BigDecimal(-1.0)));
			 ...
			 * aPurchaseOrdersBean.setDifference((BigDecimal) aObj[6]);
			 * runningBalance = runningBalance.subtract((BigDecimal) aObj[6]);
			 * }else{ aPurchaseOrdersBean.setJobName("Stock PO Received");
			 * aPurchaseOrdersBean.setQuantityOrdered((BigDecimal) aObj[6]);
			 * runningBalance = runningBalance.add((BigDecimal) aObj[6]); } } }
			 * pricemul=new BigDecimal(0.00);
			 * aPurchaseOrdersBean.setInventoryOnHand(runningBalance);
	...
	@Override
	public Prwarehouseinventory getPrwarehouseInventory(Integer PrwarehouseID, Integer prmasterID)
			throws InventoryException {
		Session aSession = null;
		Query aQuery = null;
		List<Prwarehouseinventory> aQueryList = null;
		...
		try {
			aSession = itsSessionFactory.openSession();
			aQuery = aSession.createQuery("FROM  Prwarehouseinventory where prWarehouseId=" + PrwarehouseID
					+ " and prMasterId=" + prmasterID);
			aQueryList = aQuery.list();
