...
//				addlist.add("UseCustomerscreditlimitwhencreatingjobs");
				...
				//job Setting 
				//Added by prasant #612 11-08-2016
				...
				addlist.add("RemoveLISTcolumnfromInvoicesPDF");//112
				
				addlist.add("RemoveMULTcolumnfromInvoicesPDF");//113
				addlist.add("RemoveEXTLISTcolumnfromInvoicesPDF");//114
...
//					}
					...
					//job setting added by prasant #612
					else if(i==108){
						logger.info(i+" theSysvariable.getValueString() = "+theSysvariable.getValueLong());
						if( session.getAttribute("chkfontStyleonTextEditorYN") != null ) 
							session.removeAttribute("chkfontStyleonTextEditorYN");
						session.setAttribute("chkfontStyleonTextEditorYN", theSysvariable.getValueLong());
						if( session.getAttribute("inp_fontStyleonTextEdValue") != null ) 
							session.removeAttribute("inp_fontStyleonTextEdValue");
						session.setAttribute("inp_fontStyleonTextEdValue", theSysvariable.getValueString());
					}
					else if(i==109){
						logger.info(i+" theSysvariable.getValueString() = "+theSysvariable.getValueLong());
						if( session.getAttribute("chkfontSizeonTextEditorYN") != null ) 
							session.removeAttribute("chkfontSizeonTextEditorYN");
						session.setAttribute("chkfontSizeonTextEditorYN", theSysvariable.getValueLong());
						if( session.getAttribute("inp_fontSizeonTextEdValue") != null ) 
							session.removeAttribute("inp_fontSizeonTextEdValue");
						session.setAttribute("inp_fontSizeonTextEdValue", theSysvariable.getValueString());
					}
					else if(i==110){
						logger.info(i+" theSysvariable.getValueString() = "+theSysvariable.getValueLong());
					...
					//added by prasant 
						...
						logger.info(i+" theSysvariable.getValueString() = "+theSysvariable.getValueLong());
						model.addAttribute("chk_cusRemListfmSalOrdpdfYes", theSysvariable.getValueLong());
					}
					else if(i==113){
						logger.info(i+" theSysvariable.getValueString() = "+theSysvariable.getValueLong());
						model.addAttribute("chk_cusRemMultfmallInvoicepdfYes", theSysvariable.getValueLong());
					}
					else if(i==114){
						logger.info(i+" theSysvariable.getValueString() = "+theSysvariable.getValueLong());
						model.addAttribute("chk_cusRemExtfmallInvoicepdfYes", theSysvariable.getValueLong());
					}
	...
	public String getJobListPage(
		...
		try {
			...
			if (theJobNumber != null && theToken.equalsIgnoreCase("view")) {
				...
				aJobHistory.setTsUserLoginID(aUserBean.getUserId());
	...
	public String getInventoryDetailsPage(
			...
			@RequestParam(value = "itemCode", required = false) String theItemCode,
			@RequestParam(value = "token", required = false) String theToken,
			@RequestParam(value = "searchId", required = false) String NsearchId,
			ModelMap model, HttpSession theSession, HttpServletRequest request,
			HttpServletResponse response, ModelMap theModel)
